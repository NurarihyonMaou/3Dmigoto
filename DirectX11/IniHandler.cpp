...
std::wstring global_ini_namespace{};
...
#define IniWarningWithNamespace(ini_namespace, fmt, ...) do { \
	if (G->gShowWarnings) { \
		ini_warned = true; \
		std::string warning_string = static_cast<string>(fmt)+" - "+wstringToString(ini_namespace); \
		const char* c_str_literal = warning_string.c_str(); \
		char* edited_fmt = const_cast<char*>(c_str_literal);\
		LogOverlay(LOG_WARNING, edited_fmt, __VA_ARGS__); \
	} \
} while (0)
...
{
	global_ini_namespace = *ini_namespace;
	...
	const wstring* ini_path)
{
	...
	*section = wline->substr(first, last - first + 1);
	...
	if (!inserted && !allow_duplicate_sections) {
		IniWarningWithNamespace(global_ini_namespace, "WARNING: Duplicate section found in d3dx.ini: [%S]\n",
			section->c_str());
		section->clear();
		*section_vector = NULL;
		return;
	...
	*section_vector = &ini_sections[*section].kv_vec;
	...
	if (IsCommandListSection(section->c_str())) {
		...
		IniWarningWithNamespace(global_ini_namespace, "WARNING: Unknown section in d3dx.ini: [%S]\n", section->c_str(), ini_namespace);
	}
...
static bool ParseIniPreamble(wstring* wline, wstring* ini_namespace)
{
	...
	IniWarningWithNamespace(global_ini_namespace, "WARNING: d3dx.ini entry outside of section: %S\n",
		wline->c_str());
	return true;
}
	...
	if (delim != wline->npos) {
		...
		if (warn_duplicates == 2) {
			...
			if ((warn_duplicates == 1) && !inserted && !whitelisted_duplicate_key(section->c_str(), key.c_str())) {
				IniWarningWithNamespace(global_ini_namespace, "WARNING: Duplicate key found in d3dx.ini: [%S] %S\n",
		...
		if (warn_lines_without_equals) {
			IniWarningWithNamespace(global_ini_namespace, "WARNING: Malformed line in d3dx.ini: [%S] \"%S\"\n",
...
static bool matches_globbing_vector(wchar_t* filename, vector<pcre2_code*>& patterns) {
	...
	afilename = to_utf8(filename); // to_bytes = to utf8
	...
	wchar_t* ret, unsigned size)
{
	...
	std::wstring_view section_str(section);
	auto sectionIt = ini_sections.find(section);

	if (sectionIt != ini_sections.end()) {
		// Check if the key exists within the section
		auto& kv_map = sectionIt->second.kv_map;

		std::wstring_view section_str(key);
		auto keyIt = kv_map.find(key);
		if (keyIt != kv_map.end()) {
			// Key found, copy the value
			const std::wstring& val = keyIt->second;
			...
			// Key not found, use the default value if provided
			if (def) {
				if (wcscpy_s(ret, size, def)) {
					DoubleBeepExit();  // Handle the programming error
				}
				else {
					rc = (int)wcslen(ret);
				}
		...
		// Section not found, use the default value if provided
		if (def) {
			if (wcscpy_s(ret, size, def)) {
				DoubleBeepExit();  // Handle the programming error
			}
...
bool GetIniString(const wchar_t* section, const wchar_t* key, const wchar_t* def, std::string* ret)
{
	...
	if (sectionIt != ini_sections.end()) {
		...
		if (keyIt != kv_map.end()) {
			...
			// No default value, return an empty string
			wret = L"";
		}
	}
	else if (def) {
		// Section not found, use the default value
		wret = def;
	}
	else {
		// No default value, return an empty string
	...
	wchar_t* prefix, wchar_t* names[], int names_len, int first)
{
	...
	if (GetIniString(section, key, 0, val, MAX_PATH)) {
		try {
			...
			IniWarningWithNamespace(global_ini_namespace, "WARNING: Unrecognised %S=%S\n", key, val);
		}
	}
	...
	struct EnumName_t<const wchar_t*, T>* enum_names)
{
	...
	if (GetIniString(section, key, 0, val, MAX_PATH)) {
		...
		if (tmp_found) {
			...
			IniWarningWithNamespace(global_ini_namespace, "WARNING: Unknown %S=%S\n", key, val);
		}
	}
		...
		for (i = include_sections.begin(); i != include_sections.end(); i++) {
			...
			for (entry = section->begin(); entry < section->end(); entry++) {
				...
				if (!wcscmp(key->c_str(), L"include")) {
					...
					IniWarningWithNamespace(entry->ini_namespace, "WARNING: Unrecognised entry: %S=%S\n", key->c_str(), rel_path.c_str());
				}
			}
		}
...
static std::vector<T> string_to_typed_array(std::istringstream* tokens)
{
	...
	while (std::getline(*tokens, token, ' ')) {
		...
		IniWarningWithNamespace(global_ini_namespace, "WARNING: Parse error: %s\n", token.c_str());
	}
	...
	wchar_t* whitelist[], bool register_command_lists = true)
{
	...
	for (entry = section->begin(); entry < section->end(); entry++) {
		...
		if (entry->ini_namespace == G->user_config && !G->user_config.empty()) {
			...
			LogInfo("WARNING: Unrecognised entry in %S: %S\n", G->user_config.c_str(), raw_line->c_str(), entry->ini_namespace);
			continue;
		}

		IniWarningWithNamespace(entry->ini_namespace, "WARNING: Unrecognised entry: %S\n", raw_line->c_str());
	}
...
static void ParseConstantsSection()
{
	...
	for (next = section->begin(), entry = next; entry < section->end(); entry = next) {
		...
		if (!val->empty()) {
			...
			if (len != val->length()) {
				IniWarning("WARNING: Floating point parse error: %S=%S\n", key->c_str(), val->c_str(), ini_namespace);
				continue;
			}
		}
